...
export const AUTH_SESSION_COOKIE_NAME = "venshield-session";
...
function getSessionSecret(): string {
  ...
  if (!secret) {
    ...
    return "dev-only-venshield-session-secret-change-me";
  }
  if (secret.length < 32) {
    throw new Error(
...
export async function verifySessionToken(token: string | null | undefined): Promise<SessionClaims | null> {
  ...
  try {
    ...
    if (claims.type !== "venshield-session" || claims.exp <= Date.now()) return null;
