...
export async function forceResetVendorPasswordAction(
  ...
  const setupToken = cookieStore.get("venshield-vendor-setup")?.value;
  if (!setupToken) {
    return { error: "Session expired. Please log in again." };
  }

  const vendorId = cookieStore.get("venshield-vendor-id")?.value;
  if (!vendorId) {
    return { error: "Session expired. Please log in again." };
  }
  ...
  try {
    ...
    cookieStore.delete("venshield-vendor-setup");
    cookieStore.delete("venshield-vendor-id");
    cookieStore.delete("venshield-vendor-token");
    cookieStore.delete("venshield-vendor-code-exp");
