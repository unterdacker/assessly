...
function buildRemediationPrompt(args: {
  ...
  return [
    `You are a Senior GRC Officer for Venshield. The current user interface is set to ${locale}. You MUST generate the remediation plan and the email draft entirely in ${localeLabel}.`,
...
> {
  ...
  const localeSystemPrompt = `You are a Senior GRC Officer for Venshield. The current user interface is set to ${args.locale}. You MUST generate the remediation plan and the email draft entirely in ${localeLabel}. Ensure NIS2 terminology is professionally translated.`;
  ...
  try {
    ...
    if (result.gaps.length === 0) {
      const fallback =
        locale === "de"
          ? [
              "Betreff: Venshield NIS2 Pruefung - Bestaetigung der Kontrollabdeckung",
              "",
          ...
          : [
              "Subject: Venshield NIS2 Review - Confirmation of Control Coverage",
              "",
