 ...
 *       Unix-epoch timestamp (venshield-forensic-bundle-{ts}.json).
 ...
 *       Unix-epoch timestamp (venshield-forensic-bundle-{ts}.csv).
 ...
 * "venshield-forensic-bundle-{timestamp}.{ext}" Content-Disposition filename,
...
function extractTimestampFromDisposition(
  ...
  const match = disposition.match(/venshield-forensic-bundle-(\d+)\.(json|csv)/);
...
test.describe("Forensic Bundle — JSON format", () => {
  ...
  test("JSON Content-Disposition filename embeds a current Unix-epoch timestamp", async ({
    ...
    expect(
      ts,
      `Content-Disposition "${disposition}" does not contain the expected venshield-forensic-bundle-{timestamp}.json pattern`,
    ).not.toBeNull();
...
test.describe("Forensic Bundle — CSV format", () => {
  ...
  test("CSV Content-Disposition filename embeds a current Unix-epoch timestamp", async ({
    ...
    expect(
      ts,
      `Content-Disposition "${disposition}" does not contain the expected venshield-forensic-bundle-{timestamp}.csv pattern`,
    ).not.toBeNull();
